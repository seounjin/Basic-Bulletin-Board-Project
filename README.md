...
-----------
  ...
  + ### 서버    
       ...
       사용자가 로그인을 위해 서버에 요청하여 정보를 전달하면 서버는 전달받은 그 정보를 암호화 하여 기존에 암호화되어 있는 정보와 비교하고 
       맞을 시 JWT토큰, 생성시간을 데이터베이스에 저장 후 쿠키, 토큰 정보를 유저에게 보냄.
       ...
       로그인하여 이용할 수 있는 페이지의 경우 클라이언트에서 각각의 페이지를 클릭할 때마다 AUTH로써 쿠키에 저장되어 있는 
       사용자 JWT를 복호화해서 토큰을 비교함으로써 사용자 인증의 유효성 판단하게됨 유효한 경우 해당 페이지로 이동할 수 있게 됨.
